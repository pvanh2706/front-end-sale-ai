<template>
  <div class="relative" ref="dropdownRef">
    <button
      class="relative flex items-center justify-center transition-colors rounded-full h-11 w-11 border border-[var(--color-sidebar-active-border)] text-[var(--color-sidebar-text)] bg-transparent hover:bg-primary-100 dark:hover:bg-white/10 dark:hover:text-white"
      @click="toggleDropdown"
    >
      <span
        :class="{ hidden: !notifying, flex: notifying }"
        class="absolute right-0 top-0.5 z-1 h-2 w-2 rounded-full bg-warning-500"
      >
        <span
          class="absolute inline-flex w-full h-full bg-warning-500 rounded-full opacity-75 -z-1 animate-ping"
        ></span>
      </span>
      <svg
        class="fill-current"
