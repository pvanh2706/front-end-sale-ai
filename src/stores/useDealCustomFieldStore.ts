...
const STORAGE_KEY = 'salio_deal_custom_fields'
...
export const useDealCustomFieldStore = defineStore('dealCustomField', () => {
  const data = load()
  ...
  function addField(sectionId: string, label: string, type: CustomFieldType, options?: string[]): void {
    customFields.value.push({ id: 'cf_' + Date.now(), sectionId, label, type, required: false, options })
    save()
  }
