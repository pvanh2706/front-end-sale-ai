...
const STORAGE_KEY = 'salio_lead_custom_fields'
...
export const useLeadCustomFieldStore = defineStore('leadCustomField', () => {
  const data = load()
  ...
  function addSection(name: string): CustomSection {
    const section: CustomSection = { id: 'custom_' + Date.now(), name }
  ...
  function addField(sectionId: string, label: string, type: CustomFieldType, options?: string[]): void {
    customFields.value.push({ id: 'lcf_' + Date.now(), sectionId, label, type, required: false, options })
    save()
  }
